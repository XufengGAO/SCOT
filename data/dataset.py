...
class CorrespondenceDataset(Dataset):
    ...
    def __init__(self, benchmark, datapath, thres, device, split, imside=(256,256)):
        ...
        self.range_ts = torch.arange(self.max_pts).to(self.device)
    ...
    def __getitem__(self, idx):
        ...
        sample['src_img'] = self.transform(src_pil).to(self.device) # totensor, HxW
        sample['trg_img'] = self.transform(trg_pil).to(self.device)
        ...
        sample['src_ratio'] = (self.imside / sample['src_imsize']).to(self.device)
        sample['trg_ratio'] = (self.imside / sample['trg_imsize']).to(self.device)
    ...
    def match_idx(self, kps, n_pts):
        ...
        return nearest_idx.to(self.device)
