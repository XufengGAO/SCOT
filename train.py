...
def train(epoch, model, dataloader, loss_func, optimizer, args):
    ...
    for step, data in enumerate(dataloader):
        ...
        if args.loss == "strong_ce":
            ...
            loss = loss_func.compute_loss(cross_sim, data['flow'], model.feat_size)
        elif args.loss == 'weak':
...
def validate(epoch, model, dataloader, args):
...
def make_dataloader(args, rank=0, world_size=1):
...
def main(args):
    ...
    ####    2. make dataloader  ####
    trn_dl, val_dl = make_dataloader(args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # training
        trn_loss, trn_pck = train(epoch, model, trn_dl, loss_func, optimizer, args)
        ...
        if args.split not in ["val", "trnval"]:
            with torch.no_grad():
                val_loss, val_pck = validate(epoch, model, val_dl, args)
