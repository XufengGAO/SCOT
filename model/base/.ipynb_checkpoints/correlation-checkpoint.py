...
class Correlation:
    ...
    def mutual_nn_filter(cls, correlation_matrix):
        ...
        corr_src_max = torch.max(correlation_matrix, dim=2, keepdim=True)[0] # max for ech row
        corr_trg_max = torch.max(correlation_matrix, dim=1, keepdim=True)[0] # max for each col
        corr_src_max[corr_src_max <= 1e-30] += 1e-30
        corr_trg_max[corr_trg_max <= 1e-30] += 1e-30

        corr_src = correlation_matrix / corr_src_max
        corr_trg = correlation_matrix / corr_trg_max
        del corr_src_max, corr_trg_max
